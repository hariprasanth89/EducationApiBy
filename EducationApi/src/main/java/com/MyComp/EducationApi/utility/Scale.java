...
public class Scale {
	 ...
	 * <p/>
	 * Value is "<code>imgScale.debug</code>".
	 */
	public static final String DEBUG_PROPERTY_NAME = "imgScale.debug";

	/**
	 * System property name used to define a custom log prefix.
	 * <p/>
	 * Value is "<code>imgScale.logPrefix</code>".
	 */
	public static final String LOG_PREFIX_PROPERTY_NAME = "imgScale.logPrefix";

	/**
	 * Flag used to indicate if debugging output has been enabled by setting the
	 * "<code>imgScale.debug</code>" system property to <code>true</code>. This
	 * value will be <code>false</code> if the "<code>imgScale.debug</code>"
	 * system property is undefined or set to <code>false</code>.
	 * <p/>
	 * This property can be set on startup with:<br/>
	 * <code>
	 * -DimgScale.debug=true
	 ...
	 * This property can be set on startup with:<br/>
	 * <code>
	 * -DimgScale.logPrefix=&lt;YOUR PREFIX HERE&gt;
	 * </code> or by calling {@link System#setProperty(String, String)} to set a
	 * new property value for {@link #LOG_PREFIX_PROPERTY_NAME} before this
	 * class is loaded.
	 * <p/>
	 * Default value is "<code>[imgScale] </code>" (including the space).
	 */
	public static final String LOG_PREFIX = System.getProperty(
			LOG_PREFIX_PROPERTY_NAME, "[imgScale] ");
	 ...
	 * JDK that imgScale is meant to replace).
	 * <p/>
	 ...
	 * imgScale allows the {@link ConvolveOp} to create its own destination
	...
	public static enum Method {
		 ...
		 * {@link Scale#THRESHOLD_QUALITY_BALANCED} or
		 * {@link Scale#THRESHOLD_BALANCED_SPEED} thresholds as cut-offs to
	...
	public static enum Mode {
		 ...
		 * 
		 * See "Image Proportions" in the {@link Scale} class description for
		 * more detail.
		 */
		AUTOMATIC,
		/**
	 ...
	 *             defined on the imgScale API to make it easier for callers to
	 *             catch the exception (if they are passing along optional ops
	 *             to be applied). imgScale takes detailed steps to avoid the
	 ...
	 *            The image that will be scaled.
	 * @param resizeMode
	 *            Used to indicate how imgScale should calculate the final
	 ...
	 *            {@link Mode#AUTOMATIC} is passed in, imgScale will calculate
    ...
    if (DEBUG)
      t = System.currentTimeMillis();
		...
		if (scalingMethod == Scale.Method.AUTOMATIC)
		...
		if (scalingMethod == Scale.Method.SPEED) {
			result = scaleImage(src, targetWidth, targetHeight,
					RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR);
		} else if (scalingMethod == Scale.Method.BALANCED) {
			result = scaleImage(src, targetWidth, targetHeight,
					RenderingHints.VALUE_INTERPOLATION_BILINEAR);
		} else if (scalingMethod == Scale.Method.QUALITY
				|| scalingMethod == Scale.Method.ULTRA_QUALITY) {
			/*
	 ...
	 * code inside of the imgScale library.
	 * <p/>
	 ...
	 * {@link Scale#DEBUG} is <code>true</code>; otherwise the VM will be
	 ...
	 * 
	 * @see Scale#LOG_PREFIX
	 * @see Scale#LOG_PREFIX_PROPERTY_NAME
	 */
	protected static void log(int depth, String message, Object... params) {
		if (Scale.DEBUG) {
			System.out.print(Scale.LOG_PREFIX);
	 ...
	 * unmodified.
	 * <p/>
	 * This method is meant to be used by any calling code (imgScale's or
	 ...
	 * method utilizes the {@link Scale#THRESHOLD_QUALITY_BALANCED} and
	 * {@link Scale#THRESHOLD_BALANCED_SPEED} thresholds when selecting which
	...
	protected static Method determineScalingMethod(int targetWidth,
		...
		if (length <= Scale.THRESHOLD_QUALITY_BALANCED)
			result = Method.QUALITY;
		else if (length <= Scale.THRESHOLD_BALANCED_SPEED)
	 ...
	 *            imgScale) to use for this incremental scaling operation.
	 * @param interpolationHintValue
	 *            The {@link RenderingHints} interpolation value used to
	 *            indicate the method that {@link Graphics2D} should use when
	 *            scaling the image.
